...
// END FETCH STUDENTS METHOD
    ...
    public function check_exists_absence($student_id, $day) {
        $select    = "SELECT day FROM absence WHERE student_id = '$student_id' AND day = '$day' ";
        if ($query = $this->connect->query($select)) {
            $row   = mysqli_fetch_assoc($query);
            if (!empty($row)) {
                return true;
    ...
    public function insert_absence_by_schoolroom() {
        $data = $_POST;
        if (isset($_REQUEST['submit'])) {
                if (isset($_POST['student_id'])         &&
                    isset($_POST['schoolroom_id'])&&
                    ...
                    if (!empty($_POST['student_id'])         &&
                        ...
                        !empty($_POST['day'])) {
                            foreach ($_POST['student_id'] as $key => $value) {
                                ...
                                $day           = $_POST['day'][$key];
                                $data[]        = $value;
                                if (!$this->check_exists_absence($student_id, $day)) {
                                    $insert = "INSERT INTO $this->tables 
                                    ( student_id, schoolroom_id, status, day ) 
                                    VALUES 
                                    ('$student_id', '$schoolroom_id', '$status', '$day')";
                                    if ($query = $this->connect->query($insert)) {
                                        ...
                                        echo "<script>alert('$day is added before')</script>";
                                }
                            }
                    } else {
                        echo "<script>alert('some field is empty')</script>";
